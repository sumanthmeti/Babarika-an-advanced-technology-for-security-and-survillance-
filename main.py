...
########################################## Login window and acces to main page #########################################***************************#
        ...
        def log_page():
                ...
                if (usid,usps) == (U_entry,P_entry) :
                    ...
                    self.log_add = Button(self.subframe2,text = 'Add criminal',width=15,bg='#99CCCC',fg='#000033',font = 'Areal 25 bold',bd=3,command=add_criminal)
                    self.log_add.place(x=50,y=150)      #Display button in main
                    self.log_display= Button(self.subframe2,text = 'Display',width=15,bg='#99CCCC',fg='#000000',font = 'Areal 25 bold',bd=3,command=display_criminal)
                    self.log_display.place(x=50,y=250)      #delete button in main
                    self.log_delete = Button(self.subframe2,text = 'Delete',width=15,bg='#99CCCC',fg='#000000',font = 'Areal 25 bold',bd=3,command=delete_criminal)
                    self.log_delete.place(x=50,y=350)      #Analise button in main
                    self.log_analise = Button(self.subframe2,text = 'Analyze',width=15,bg='#000033',fg='#CCFFFF',font = 'Areal 25 bold',bd=7,command=opencamera)
...
########################################### Adding Terrorist form in after login ###################################################################
        ...
        def add_criminal():
            ...
            def criminal_database():
                ...
                criminal =sqlite3.connect('terrorist.db')
                tcur=criminal.cursor()
                try:
                    ...
                    self.subframe1 = Frame(self.bottom,bg='white',width=968,height=600,relief=SUNKEN,bd=2)
                    self.subframe1.pack(fill=X,side=LEFT)
                    self.thankyou=Label(self.subframe1,text='criminal Details  Added to Data base Sucessfully',font='Areal 13 bold',fg='#3333CC')
                    self.thankyou.place(x=320,y=200)
                    
                    engine.say("Please wait \n connecting to data base \n details accepted \nupload  an    image   of a criminal  for   future   predictions \n thankyou")
                    engine.runAndWait()
                    #terrorists.close()
                    
                    ib=Button(self.subframe1,text='Upload criminal Photo',width=25,font='Areal 16 bold',bg='#339933',fg='#FFFFFF',command=store_image).place(x=350,y=450)
            ...
            t_name = ttk.Label(self.subframe1,text='criminal Name',font='Areal 13 bold').place(x=250,y=140)
            ...
            t_country = ttk.Label(self.subframe1,text='state',font='Areal 13 bold').place(x=350,y=340)
            t_accused = ttk.Label(self.subframe1,text='About',font='Areal 13 bold').place(x=250,y=400)
            ...
            ae=ttk.Entry(self.subframe1,textvariable=AE,width=40,font='Areal 13 ').place(x=430,y=240)
                        #terrorist country
            ce = ttk.Combobox(self.subframe1,textvariable=CE,width=58,values=('Karnataka','delhi','tamilnadu','mumbai','goa','kerala','punjab','sikkim'))
            ...
            getabout = ttk.Entry(self.subframe1,width=40,font='Areal13 ',textvar=aboutter).place(x=430,y=400)
                # Image upload button
            #ib=Button(self.subframe1,text='Upload',width=8,bg='#CCCCCC',command=store_image).place(x=430,y=480)
            Register=Button(self.subframe1,text=' Register',width=15,bg='#336600',fg='black',bd=3,font='Areal 10 bold',command=criminal_database).place(x=460,y=480)
            Reset=Button(self.subframe1,text='Reset ',command=add_criminal,width=15,bg='#9999CC',bd=3).place(x=620,y=480)
...
########################################   Display Terrorist Details by Search  ##############################################################################
        ...
        def tbsearch():
            ...
            try:
                ...
                self.trname=Label(self.subframe1,text=("criminal Name   :"),font='Areal 13 bold').place(x=370,y=150)
                ...
                self.trcountry=Label(self.subframe1,text=("state              :"),font='Areal 13 bold').place(x=370,y=300)
        ...
        def display_criminal():                #after search creating new frames and label and search DB
            #frames created
            chdir=os.chdir(os.path.dirname(os.path.abspath(__file__)))
            self.subframe1.destroy()
            self.subframe1 = Frame(self.bottom,bg='white',width=968,height=600,relief=SUNKEN,bd=2)
            ...
            #----------------------------------------------------------------
            self.SRE=StringVar()
            self.searchtitle=Label(self.subframe1,text="  Enter criminal  Name ",font="Areal 16 bold",fg='#009999').place(x=50,y=230)
...
#***************************************** Delete Terrorist  ***************************************************************** '''
        ...
        def delete_criminal():
            ...
            self.searchbtn=Button(self.subframe1,text='Search',bg='#336699',font='Areal 10 bold',bd=5,command=deleting_criminal).place(x=125,y=305)
...
#############################################  Officer Registration  ######################################################################################
        ...
        def empreg():
            ...
            title =Label(self.subframe1,text="Police Registraion Desk",fg="#3366FF",font='Areal 25 bold')
            ...
            odp=ttk.Combobox(self.subframe1,textvariable=dep,font='Areal 13 ',values=("Superintendent of Police","Additional Superintendent of Police","Deputy SP ","Assistant Sub-Inspector of Police","Inspector of Police","Sub-Inspector of Police"))
...
def main():
    ...
    root.title('Facial Recognition To catch criminals in public Areas')
